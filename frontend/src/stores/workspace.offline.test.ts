...
describe('workspace store offline behavior', () => {
  it('falls back to a saved snapshot when refreshGroup runs offline', async () => {
    const fetchMock = vi.fn().mockImplementation((url: string) => {
      ...
      if (url.includes('/groups/g1/access')) return Promise.resolve(envelope({ permissions: ['group.view', 'ledger.expenses.read', 'ledger.subscriptions.read', 'ledger.settlements.read'] }))
