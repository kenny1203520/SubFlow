...
export const useWorkspaceStore=defineStore('workspace',()=>{
  ...
  async function refreshGroup(){if(!currentGroupId.value)return;const id=currentGroupId.value;await run(async()=>{const [m,i,s,e,d]=await Promise.all([api.get<Membership[]>(`/groups/${id}/members?perPage=100`),api.get<Invitation[]>(`/groups/${id}/invitations?perPage=100`),api.get<Subscription[]>(`/groups/${id}/subscriptions?perPage=100`),api.get<Expense[]>(`/groups/${id}/expenses?perPage=100`),api.get<DashboardSummary>(`/groups/${id}/summary`)]);members.value=m.data;invitations.value=i.data;subscriptions.value=s.data;expenses.value=e.data;summary.value=d.data})}
  ...
  return{groups,currentGroupId,currentGroup,members,invitations,subscriptions,expenses,summary,loading,error,permissionDenied,loadGroups,selectGroup,refreshGroup,createGroup,invite,resendInvitation,revokeInvitation,acceptInvitation,addSubscription,deleteSubscription,addExpense,deleteExpense,clear}
})
