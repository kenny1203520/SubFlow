...
async function create() { reset(); if(!personal.value) workspace.members.forEach(member=>{form.participants[member.userId]=true}); drawer.value = true; await loadFormCategories() }
...
<template>
  <section class="page ledger-page">
    ...
    <AppDrawer :open="drawer" :title="tr(editingId?'editSubscription':'createSubscription')" @close="drawer=false"><form class="form-card ledger-form" @submit.prevent="submit">
      ...
      <section class="ledger-form-section"><div class="ledger-section-heading"><strong>{{tr('amount')}} · {{tr('currency')}}</strong></div><div class="ledger-form-grid"><label>{{tr('amount')}}<input v-model="form.amount" type="number" min="0" step="0.01" required></label><label>{{tr('currency')}}<CurrencySelect v-model="form.currency" :currencies="workspace.currencies"/></label><label v-if="!personal">{{tr('payer')}}<PayerSelect v-model="form.paidBy" :members="workspace.members" :self-id="auth.record?.id"/></label></div></section>
