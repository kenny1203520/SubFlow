...
async function addGroupCategory(){if(!categoryName.value.trim())return;await workspace.createCategory('group',categoryName.value.trim(),workspace.currentGroupId);categoryName.value=''}
...
<template><section class="page">
  ...
  <template v-else>
    ...
    <section class="card form-card"><div class="card-title"><h2>{{tr('category')}}</h2></div><div class="inline-create"><input v-model="categoryName" :placeholder="tr('addCategory')"><button class="primary" type="button" @click="addGroupCategory">{{tr('add')}}</button></div><div class="data-list"><article v-for="item in (workspace.categories||[]).filter(v=>v.scope==='group')" :key="item.id" class="data-row"><span class="grow">{{item.customName}}</span><button class="ghost danger-text" @click="workspace.archiveCategory(item.id)">{{tr('remove')}}</button></article></div></section>
  </template>
