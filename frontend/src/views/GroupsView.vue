...
async function remove() {
    if (
        ...
        !confirm(`確定刪除「${workspace.currentGroup.name}」？此操作無法復原。`)
    )
        return;
    ...
    <section class="page">
        ...
        <div class="two-column">
            <div class="card">
                ...
                <div v-if="workspace.groups.length" class="group-grid">
                    <div v-for="group in workspace.groups" :key="group.id" class="group-tile-wrap">
                        <button class="group-tile" :class="{ selected: group.id === workspace.currentGroupId }"
                            @click="open(group)">
                            <span class="group-color" :style="{ background: group.color }"></span>
                            <strong>{{ group.name }}</strong>
                            <small>{{ group.currency }} ·
                                {{ group.description || "尚無說明" }}</small>
                        </button>
                        <button v-if="group.id === workspace.currentGroupId && workspace.isOwner"
                            class="ghost edit-link" @click="edit(group)">
                            編輯
                        </button>
                    </div>
                </div>
            ...
            <form class="card form-card" @submit.prevent="submit">
                ...
                <label>簡短說明<textarea v-model="form.description" rows="3" placeholder="這個群組用來管理…"></textarea>
                </label>
                <div class="form-row">
                    <label>幣別<select v-model="form.currency">
                            <option>TWD</option>
                            <option>USD</option>
                            <option>JPY</option>
                            <option>EUR</option>
                ...
                <div class="form-actions">
                    ...
                    <button v-if="editing" type="button" class="ghost" @click="reset">
                        取消
                    </button>
                    <button v-if="editing" type="button" class="ghost danger-text push-right" @click="remove">
                        刪除群組
                    </button>
                </div>
            </form>
