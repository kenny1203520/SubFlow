...
const canWrite=computed(()=>personal.value||workspace.groupPermissions.includes('ledger.expenses.write'))
const canDelete=computed(()=>personal.value||workspace.groupPermissions.includes('ledger.expenses.delete'))
function canEditItem(item:Expense){return personal.value?!item.groupId:canWrite.value}
function canDeleteItem(item:Expense){return personal.value?!item.groupId:canDelete.value}
const canExport=computed(()=>personal.value||['group.view','ledger.expenses.read','ledger.subscriptions.read','ledger.settlements.read'].every(permission=>workspace.groupPermissions.includes(permission)))
...
async function remove(){if(!pendingDelete.value||!canDelete.value)return;const page=listMeta.value.page;await workspace.deleteExpense(pendingDelete.value.id);await reloadCurrentPage(page);pendingDelete.value=undefined}
