...
const open=ref(false),editingId=ref(''),pendingDelete=ref<Expense>(),formError=ref(''),rateValid=ref(true)
...
function reset(){editingId.value='';formError.value='';rateValid.value=true;Object.assign(form,{title:'',category:'',categoryId:'',amount:'',currency:workspace.currentGroup?.currency||'TWD',rateMode:'automatic',exchangeRate:'',paidBy:'',incurredOn:new Date().toISOString().slice(0,10),notes:'',splitMode:'equal',participants:{},values:{}})}
...
<template><section class="page ledger-page"><PersonalLedgerNav v-if="personal"/><div class="page-heading"><div><p class="eyebrow">{{tr(personal?'expensePersonal':'splitExpenses')}}</p><h1>{{tr(personal?'expensePersonal':'expenseGroup')}}</h1><p>{{tr('expenseDesc')}}</p></div><button class="primary" @click="create">{{tr('createExpense')}}</button></div>
  ...
  <AppDrawer :open="open" :title="tr(editingId?'editExpense':'createExpense')" @close="open=false"><form class="form-card" @submit.prevent="submit">
    ...
    <label>{{tr('notes')}}<textarea v-model="form.notes" rows="3"></textarea></label><div class="form-actions"><button type="button" class="ghost" @click="open=false">{{tr('cancel')}}</button><button class="primary" :disabled="workspace.loading||!splitValid||!rateValid">{{tr(editingId?'saveChanges':'createExpense')}}</button></div></form></AppDrawer>
