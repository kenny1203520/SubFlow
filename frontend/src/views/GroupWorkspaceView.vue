...
const canReadIncomes = computed(() => workspace.groupPermissions.includes('ledger.incomes.read'))
...
const canViewLedger = computed(() => canViewGroup.value && canReadExpenses.value && canReadIncomes.value && canReadSubscriptions.value)
const routePermissions = computed<Record<string, string[]>>(() => ({
  ...
  'group-ledger': ['group.view', 'ledger.expenses.read', 'ledger.incomes.read', 'ledger.subscriptions.read'],
  'group-expenses': ['ledger.expenses.read'],
  'group-incomes': ['ledger.incomes.read'],
...
<template>
  <section class="group-workspace">
    <div class="group-workspace-head">
      <RouterLink class="back-link" to="/groups" :aria-label="tr('backToGroups')"><span aria-hidden="true">&lsaquo;</span>{{ tr('allGroupsLink') }}</RouterLink>
      <div class="group-workspace-identity">
        ...
        <div v-if="group" class="workspace-facts">
          <span>{{ currencyLabel(group.currency) }}</span><span>{{ timezoneLabel(group.timezone) }}</span><span>{{ tr('memberCount', { count: workspace.members.length }) }}</span>
          <RouterLink v-if="canManageSettings" :to="`/groups/${groupId}/settings`">{{ tr('settings') }}</RouterLink>
        </div>
      </div>
    </div>
    <nav class="group-tabs" :aria-label="tr('groupWorkspace')">
      <RouterLink v-if="canViewDashboard" :to="`/groups/${groupId}/overview`">{{ tr('groupOverview') }}</RouterLink>
      <RouterLink v-if="canViewLedger" :to="`/groups/${groupId}/ledger`">{{ tr('ledger') }}</RouterLink>
      <RouterLink v-if="canReadExpenses" :to="`/groups/${groupId}/expenses`">{{ tr('splitExpenses') }}</RouterLink>
      <RouterLink v-if="canReadIncomes" :to="`/groups/${groupId}/incomes`">{{ tr('groupIncomes') }}</RouterLink>
      <RouterLink v-if="canReadSubscriptions" :to="`/groups/${groupId}/subscriptions`">{{ tr('manageSubscriptions') }}</RouterLink>
      <RouterLink v-if="canViewGroup" :to="`/groups/${groupId}/members`">{{ tr('members') }}</RouterLink>
