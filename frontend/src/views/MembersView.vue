...
<template>
  <section class="page">
    <div class="page-heading"><div><p class="eyebrow">PEOPLE</p><h1>成員與邀請</h1><p>邀請連結有效七天，只能由相同 Email 的帳號接受一次。</p></div></div>
    <div class="two-column">
      <div class="card">
        <h2>群組成員</h2>
        <div v-if="workspace.members.length" class="rows">
          <div v-for="member in workspace.members" :key="member.id" class="row">
            <div class="avatar">{{ (member.user?.name || member.user?.email || '?').slice(0, 1).toUpperCase() }}</div>
            <div class="grow"><strong>{{ member.user?.name || '未命名成員' }}</strong><small>{{ member.user?.email }}</small></div>
            <span class="pill">{{ member.role === 'owner' ? '擁有者' : '成員' }}</span>
            <button v-if="workspace.isOwner && member.role !== 'owner'" class="ghost danger-text" @click="removeMember(member.userId, member.user?.name || member.user?.email || '此成員')">移除</button>
          </div>
        </div>
        ...
        <div class="card invitations">
          ...
          <div v-if="workspace.invitations.length">
            <div v-for="item in workspace.invitations" :key="item.id" class="invite">
              <div><strong>{{ item.email }}</strong><small>{{ item.status }} · {{ new Date(item.expiresAt).toLocaleDateString('zh-TW') }}</small><a v-if="item.debugUrl" :href="item.debugUrl">開發測試連結</a></div>
              <div v-if="item.status === 'pending' || item.status === 'delivery_failed'" class="actions">
                ...
                <button class="ghost danger-text" @click="workspace.revokeInvitation(item.id)">撤銷</button>
              </div>
            </div>
          </div>
      ...
      <div v-else class="card owner-note"><h2>成員管理</h2><p>只有群組擁有者可以邀請或移除成員。</p></div>
    </div>
  </section>
</template>
