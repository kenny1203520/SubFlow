...
<template>
  <section class="page">
    ...
    <div v-if="!route.params.groupId" class="segmented scope-switch"><button :class="{active:scope==='personal'}" @click="change('personal')">個人</button><button :class="{active:scope==='group'}" :disabled="!workspace.currentGroupId" @click="change('group')">目前群組</button><button :class="{active:scope==='all'}" @click="change('all')">All groups</button></div>
