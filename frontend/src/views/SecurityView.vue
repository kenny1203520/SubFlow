            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="space-y-8 h-full flex flex-col">
                    ...
                    <section class="glass-panel p-8 relative overflow-hidden flex-none">
                        <div class="flex items-center justify-between mb-8">
                            <div class="flex items-center gap-4">
                                <div class="w-12 h-12 rounded-2xl bg-primary-100 flex items-center justify-center text-primary-600 shadow-sm">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-2 mt-1">
                                        <span :class="['status-dot', settings?.two_factor_enabled ? 'bg-success' : 'bg-slate-300 animate-pulse']"></span>
                                        <span :class="['text-xs font-bold uppercase tracking-wider', settings?.two_factor_enabled ? 'text-success' : 'text-slate-500']">
                        ...
                        <p class="text-slate-500 text-sm mb-6 leading-relaxed">
                            {{ t('security.2faDesc', 'Two-factor authentication adds an extra layer of security to your account. To log in, you will also need to provide a 6-digit code from your authenticator app.') }}
                        </p>
                        ...
                        <div v-if="isSettingUp2FA" class="animate-slide-in mt-auto">
                            <div class="p-6 rounded-2xl bg-primary-50/50 border border-primary-100">
                                <h4 class="font-bold text-primary-800 mb-4 flex items-center gap-2">
                                    <span class="w-6 h-6 rounded-full bg-primary-600 text-white flex-center text-xs">1</span>
                                ...
                                <div class="bg-white/80 p-4 rounded-xl border-2 border-dashed border-primary-200 font-mono text-center text-xl tracking-widest text-primary-700 shadow-inner mb-6 transition-all hover:bg-white select-all cursor-pointer" title="Click to select">
                                    {{ formattedSecret }}
                                </div>

                                <h4 class="font-bold text-primary-800 mb-4 flex items-center gap-2">
                                    <span class="w-6 h-6 rounded-full bg-primary-600 text-white flex-center text-xs">2</span>
                                ...
                                <div class="space-y-4">
                                    <div class="relative">
                                        ...
                                        <div class="absolute inset-y-0 right-4 flex items-center pointer-events-none text-slate-300">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-4.44-2.03c.52-1.282.909-2.607 1.155-3.974m2.812-3.162l3.414-3.414m0 0a2 2 0 10-2.828-2.828l-3.414 3.414m0 0L5.95 10.122m3.162-3.162l3.414-3.414m0 0a2 2 0 10-2.828-2.828l-3.414 3.414m0 0L5.95 10.122" />
                                            </svg>
                                        </div>
                                    </div>

                                    <div v-if="errorMsg" class="flex items-center gap-2 p-3 rounded-lg bg-danger/10 text-danger text-xs font-bold animate-pulse">
                                    ...
                                    <div class="flex gap-3">
                                        <button @click="verifyAndEnable2FA" class="btn btn-primary flex-1 shadow-lg shadow-primary-500/20">
                                            {{ t('common.actions.verify', 'Verify & Enable') }}
                                        </button>
                                        <button @click="cancelSetup" class="btn bg-white hover:bg-slate-50 text-slate-500 border border-slate-200">
                    ...
                    <section class="glass-panel p-8 flex-none bg-white/60">
                        <div class="flex items-center gap-4 mb-8">
                            <div class="w-12 h-12 rounded-2xl bg-indigo-100 flex items-center justify-center text-indigo-600 shadow-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <form @submit.prevent="changePassword" class="space-y-5">
                            <div class="space-y-4">
                                <div class="form-group">
                                    <label class="form-label text-sm font-semibold text-slate-600 block mb-1.5">{{ t('auth.oldPassword') }}</label>
                                    <div class="relative">
                                        <input :type="showOldPassword ? 'text' : 'password'" v-model="passwordForm.oldPassword" required
                                            class="glass-input w-full pr-12" :placeholder="t('auth.oldPassword')" />
                                ...
                                <div class="form-group">
                                    <label class="form-label text-sm font-semibold text-slate-600 block mb-1.5">{{ t('auth.newPassword') }}</label>
                                    <div class="relative">
                                        <input :type="showNewPassword ? 'text' : 'password'" v-model="passwordForm.newPassword" required
                                            class="glass-input w-full pr-12" :placeholder="t('auth.newPassword')" />
                                    ...
                                    <p class="text-[10px] text-slate-400 mt-1 uppercase tracking-wider">{{ t('auth.passwordComplexity') }}</p>
                                </div>

                                <div class="form-group">
                                    <label class="form-label text-sm font-semibold text-slate-600 block mb-1.5">{{ t('auth.confirmPassword') }}</label>
                                    <div class="relative">
                            ...
                            <div v-if="passwordMsg.text" :class="['flex items-center gap-2 p-4 rounded-xl text-sm font-bold animate-fade-in', passwordMsg.type === 'error' ? 'bg-danger/10 text-danger' : 'bg-success/10 text-success']">
                                <svg v-if="passwordMsg.type === 'error'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            ...
                            <button type="submit" :disabled="isChangingPassword" class="btn btn-primary w-full shadow-lg shadow-primary-500/20 h-12">
                                <svg v-if="isChangingPassword" class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                </svg>
                                <span>{{ isChangingPassword ? t('common.status.saving') : t('common.actions.update') }}</span>
                            </button>
                        </form>
                    </section>
                </div>
                ...
                <div class="space-y-8 flex flex-col">
                    <section class="glass-panel p-8 flex flex-col flex-1 h-full">
                        <div class="flex items-center gap-4 mb-8">
                            <div class="w-12 h-12 rounded-2xl bg-amber-100 flex items-center justify-center text-amber-600 shadow-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                                </svg>
                            </div>
                            <h3 class="text-xl font-bold text-slate-800">{{ t('security.devices', 'Active Sessions') }}</h3>
                        </div>

                        <div v-if="loading" class="flex-center flex-col gap-4 py-20 flex-1">
                            <div class="animate-spin rounded-full h-10 w-10 border-4 border-primary-200 border-t-primary-600"></div>
                            <p class="text-slate-400 font-medium text-sm">{{ t('common.status.loading') }}</p>
                        </div>
                        
                        <div v-else-if="sessions.length === 0" class="flex-center flex-col gap-4 py-20 flex-1 opacity-50">
                            ...
                            <p class="text-slate-400 font-bold uppercase tracking-widest text-xs">{{ t('common.noData') }}</p>
                        </div>
                        ...
                        <div v-else class="space-y-4 flex-1">
                            <div v-for="session in sessions" :key="session.id" 
                                ...
                                <div class="flex items-center gap-5 min-w-0">
                                    <div class="w-12 h-12 rounded-xl flex items-center justify-center transition-colors bg-slate-100 text-slate-400 group-hover:bg-primary-50 group-hover:text-primary-400">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                                        </svg>
                                    </div>
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="text-xs text-slate-500 font-mono mt-1 w-full truncate" :title="session.user_agent">
                                        ...
                                        <p class="text-[10px] text-slate-400 mt-0.5 opacity-75">
                                            {{ t('security.lastUsed') }}: {{ new Date(session.created_at || Date.now()).toLocaleString() }}
                                        </p>
                                    </div>
                                </div>
                        ...
                        <div class="mt-8 p-4 rounded-xl bg-amber-50 border border-amber-100 flex gap-3">
                            ...
                            <p class="text-xs text-amber-700 leading-normal">
                                ...
                                {{ t('security.revokeHint', 'Regularly audit your active sessions. If you see something suspicious, revoke access immediately.') }}
                            </p>
                        </div>
                    </section>
                </div>
