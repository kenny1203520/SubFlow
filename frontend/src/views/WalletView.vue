...
const depositWallet = ref<any>(null);
...
const submitDeposit = (amount: number) => {
    if (!depositWallet.value) return;
...
<template>
    <MainLayout>
        <div class="flex flex-col gap-8 animate-fade-in relative z-10">
            ...
            <!-- Error State -->
            <div v-if="error"
                class="glass-panel border border-red-200 bg-red-50/50 p-6 flex items-center justify-center text-red-600 gap-3">
                ...
                <button @click="fetchWallets"
                    class="btn bg-white/50 hover:bg-white text-red-600 border border-red-200 ml-4">
            ...
            <DepositModal v-if="showDepositModal" :wallet="depositWallet" @close="showDepositModal = false"
