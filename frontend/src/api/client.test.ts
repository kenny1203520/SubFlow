...
describe('ApiClient',()=>{
  ...
  it('clears auth on 401',async()=>{const unauthorized=vi.fn();vi.stubGlobal('fetch',vi.fn().mockResolvedValue(new Response(JSON.stringify({error:{code:'unauthorized',message:'登入失效'}}),{status:401})));const client=new ApiClient(()=>'',unauthorized);await expect(client.get('/groups')).rejects.toBeInstanceOf(ApiError);expect(unauthorized).toHaveBeenCalledOnce()})
