...
export interface Contact { id:string;ownerId:string;name:string;email:string;createdAt:string;updatedAt:string }
export interface Share { id:string;groupId?:string;ownerId?:string;name:string;accessMode:ShareAccessMode;enabled:boolean;expiresAt?:string;rangeMode:ShareRangeMode;rollingDays?:number;startsOn?:string;endsOn?:string;showSummary:boolean;showExpenses:boolean;showSubscriptions:boolean;showSettlements:boolean;showIdentities:boolean;showNotes:boolean;viewers?:ShareViewer[];url?:string;createdAt:string;updatedAt:string }
export type ShareRecordType = 'expense'|'subscription'|'settlement'
export interface ShareRecord extends Record<string, unknown> { type:ShareRecordType; occurredOn?:string }
export interface SharePage { name:string;currency:Currency;rangeLabel:string;showSummary:boolean;summary?:{expenseCount:number;subscriptionCount:number;settlementCount:number;expenseTotals:Record<string,number>};records?:ShareRecord[];recordCounts?:Record<'all'|'expenses'|'subscriptions'|'settlements',number>;expenses?:Array<Record<string,unknown>>;subscriptions?:Array<Record<string,unknown>>;settlements?:Array<Record<string,unknown>>;balances?:Array<Record<string,unknown>>;page?:number;perPage?:number;totalItems?:number;totalPages?:number;nextPage?:number;requiresPassword?:boolean;requiresLogin?:boolean }
...
export interface CurrencyDashboard { currency:Currency;cashInflowMinor?:number;cashOutflowMinor:number;personalShareMinor:number;reimbursableMinor:number;monthlySubscriptionMinor:number;personalMonthlySubscriptionMinor:number;activeSubscriptions:number;chargeCount?:number }
export interface MemberBalance { userId:string;amountMinor:number }
export interface DashboardSummary { month?:string;monthlySubscriptionMinor:number;monthExpenseMinor:number;monthIncomeMinor?:number;activeSubscriptions:number;upcoming:Subscription[];currencies?:CurrencyDashboard[];balances?:MemberBalance[];reportingCurrency?:Currency;originalCurrencies?:CurrencyDashboard[] }
