            ...
            <form @submit.prevent="handleSubmit">
                <div class="form-group">
                    ...
                    <input id="groupName" v-model="groupName" type="text"
                        :placeholder="t('common.placeholders.description')" required :disabled="isSubmitting" />
                </div>
                <p v-if="error" class="error-msg">{{ error }}</p>
                <div class="actions">
                    <button type="button" @click="$emit('close')" :disabled="isSubmitting" class="btn btn-secondary">{{
                        t('common.actions.cancel') }}</button>
                    <button type="submit" :disabled="isSubmitting || !groupName.trim()" class="btn btn-primary">
