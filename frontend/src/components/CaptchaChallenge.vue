...
async function render(){
  ...
  try {
    ...
    else if(provider.value==='hcaptcha'){if(!key)return;await script('https://js.hcaptcha.com/1/api.js?render=explicit');const hcaptcha=(window as any).hcaptcha;widget.value=hcaptcha.render(host.value,{sitekey:key,size:invisible?'invisible':'normal',callback:(token:string)=>{emit('update:modelValue',token);settlePending(token)},'error-callback':rejectToken,'expired-callback':rejectToken});if(invisible)hcaptcha.execute(widget.value)}
    else if(provider.value==='recaptcha'){if(!key)return;await script('https://www.google.com/recaptcha/api.js?render=explicit');const grecaptcha=(window as any).grecaptcha;widget.value=grecaptcha.render(host.value,{sitekey:key,size:invisible?'invisible':'normal',callback:(token:string)=>{emit('update:modelValue',token);settlePending(token)},'error-callback':rejectToken,'expired-callback':rejectToken});if(invisible)grecaptcha.execute(widget.value)}
