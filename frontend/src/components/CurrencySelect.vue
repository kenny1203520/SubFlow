...
const props=withDefaults(defineProps<{modelValue:Currency;currencies?:CurrencyInfo[];disabled?:boolean}>(),{currencies:()=>[{code:'TWD',digits:2},{code:'USD',digits:2},{code:'JPY',digits:0},{code:'EUR',digits:2}],disabled:false})
...
function select(value:Currency,close:()=>void){if(props.disabled)return;emit('update:modelValue',value);query.value='';close()}
...
<template>
  <BaseDropdown v-model="open" class="currency-select" :panel-label="tr('currency')" mobile-sheet @opened="focusSearch">
    <template #trigger="{ open: isOpen, toggle }"><button class="currency-trigger" type="button" :aria-expanded="isOpen" :disabled="props.disabled" @click="toggle"><span>{{selected?.label||modelValue}}</span><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m7 10 5 5 5-5"/></svg></button></template>
