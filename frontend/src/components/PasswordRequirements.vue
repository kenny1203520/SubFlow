<template>
    <div class="mt-2 text-xs bg-slate-50/80 p-3 rounded-xl border border-slate-200 shadow-sm text-left">
        ...
        <ul class="space-y-1.5">
            ...
            <li class="flex items-center gap-2 transition-colors duration-200"
                :class="lowercaseValid ? 'text-emerald-600' : 'text-slate-500'">
                <svg v-if="lowercaseValid" xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 text-emerald-500"
                    viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd"
                        d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                        clip-rule="evenodd" />
                ...
                <span>{{ t('auth.requirements.lowercase', 'One lowercase letter') }}</span>
            </li>
            <li class="flex items-center gap-2 transition-colors duration-200"
...
const strengthLabel = computed(() => {
    if (score.value <= 2) return t('auth.strength.weak', 'Weak');
    if (score.value <= 3) return t('auth.strength.fair', 'Fair');
    if (score.value <= 4) return t('auth.strength.good', 'Good');
    return t('auth.strength.strong', 'Strong');
});

const strengthColorClass = computed(() => {
    if (score.value <= 2) return { text: 'text-rose-600', bg: 'bg-rose-50', bar: 'bg-rose-500' };
    if (score.value <= 3) return { text: 'text-amber-500', bg: 'bg-amber-50', bar: 'bg-amber-400' };
    if (score.value <= 4) return { text: 'text-blue-500', bg: 'bg-blue-50', bar: 'bg-blue-400' };
