...
const props=withDefaults(defineProps<{modelValue:string;categories?:Category[]}>(),{categories:()=>[]});const emit=defineEmits<{(e:'update:modelValue',value:string):void;(e:'create',name:string):void}>();const {tr}=useI18n();const search=ref('');const creating=ref(false);const filtered=computed(()=>props.categories.filter(v=>(v.customName||v.systemKey||'').toLowerCase().includes(search.value.toLowerCase())));function label(v:Category){return categoryLabel(v,'',tr)}
</script>
