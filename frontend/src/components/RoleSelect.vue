...
const filtered=computed(()=>{const term=search.value.trim().toLocaleLowerCase();return props.roles.filter(role=>!term||`${role.name} ${role.category||''}`.toLocaleLowerCase().includes(term))})
function choose(value:string,close:()=>void){emit('update:modelValue',value);search.value='';close()}
</script>
<template><BaseDropdown v-model="open" class="role-select" :panel-label="label||tr('assignRole')" @opened="nextTick(()=>input?.focus())"><template #trigger="{toggle}"><button type="button" class="role-trigger" :aria-expanded="open" @click="toggle"><span>{{selected?.name||tr('assignRole')}}</span><span aria-hidden="true">⌄</span></button></template><template #default="{close}"><div class="role-menu"><input ref="input" v-model="search" :placeholder="tr('search')"><button v-for="role in filtered" :key="role.id" type="button" class="role-option" :class="{selected:role.id===modelValue}" @click="choose(role.id,close)"><strong>{{role.name}}</strong><small>{{role.category||tr('ungrouped')}}</small></button><p v-if="!filtered.length" class="empty-inline">{{tr('noSummary')}}</p></div></template></BaseDropdown></template>
