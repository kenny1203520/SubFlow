...
type Stores struct {
	...
	Incomes            ports.IncomeRepository
...
func New(driver string, app core.App) (Stores, error) {
	...
	case "", "pocketbase":
		...
		return Stores{s.Groups, s.Memberships, s.Invitations, s.OwnershipTransfers, s.MemberTransfers, s.Notifications, s.Subscriptions, s.Expenses, s.Incomes, s.Settlements, s.Categories, s.ExchangeRates, s.Roles, s.Audits, s.Shares, s.Contacts, s.Users, s.Settings, s.Transactions}, nil
	case "postgres", "mysql":
