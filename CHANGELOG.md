...
### Added
- 還款紀錄支援伺服器端分頁、成員／日期篩選、排序與編輯，並改善付款人、收款人、金額、日期及建立者的資訊呈現。
- 支出與訂閱清單支援伺服器端分頁及每頁筆數選擇。
- 網站名稱與 PocketBase Application name 共用同一設定，避免重複維護造成不一致。

### Fixed
- 修正群組帳本、還款、支出、訂閱、成員與角色管理的後端權限檢查，防止透過直接 API 或 URL 越權操作。
- 禁止自訂角色使用 `*` 萬用權限，角色管理者只能授與自己已擁有的權限。
- 修正大型群組成員超過 100 人時權限解析不完整的問題。
- 補齊成功與失敗安全操作的稽核交易，稽核寫入失敗時主要更新會回滾。
- 無權限的群組子頁現在會顯示一致的拒絕狀態，並隱藏不可執行的前端操作入口。
